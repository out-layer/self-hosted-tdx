...
if [ -n "${WORKER_DIGEST:-}" ]; then
  ...
  echo "    gh release view $VERSION --repo out-layer/outlayer --json body -q .body | grep -iE '\\| *worker *\\|'" >&2
  echo "    gh attestation verify oci://docker.io/outlayer/near-outlayer-worker@<digest> -R out-layer/outlayer" >&2
...
echo "  verify (on a trusted machine): gh attestation verify oci://docker.io/outlayer/near-outlayer-worker@$DIGEST -R out-layer/outlayer   (releases <= v0.1.58: --owner fastnear)"
