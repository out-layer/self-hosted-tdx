...
`30-deploy-kms.sh` deploys the per-node KMS. **Then `kms/apply-auth-simple.sh`**
(Step 4b) — `allowAnyApp:true` plus the node device allowlist (this host's `sha256(PPID)`, derived on the node by `kms/node-device-id.py`, see
